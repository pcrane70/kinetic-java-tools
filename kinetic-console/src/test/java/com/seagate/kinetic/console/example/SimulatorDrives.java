...
import com.seagate.kinetic.example.openstorage.OpenStorageClient;

/**
 * Starts (by default) 71 simulators each listens on its own ports and with its
 ...
 * @author chiaming
 */
public class SimulatorDrives {

    public static final int MAX_SIMULATOR = 2;

    private int maxSimulator = MAX_SIMULATOR;

    // base port number
    private int port = 18920;

    // base ssl port
    private int sslPort = 18990;

    public SimulatorDrives(int maxSimulator, int portBase, int sslportBase) {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int sslportBase = 18990;
        if (args.length >= 3) {
            sslportBase = Integer.parseInt(args[2]);
        }

        // max number of simulators to instantiate.
        SimulatorDrives vdrives = new SimulatorDrives(maxSimulator, portBase,
                sslportBase);
        // start the simulator
        vdrives.run();
    }
